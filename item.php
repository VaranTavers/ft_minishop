			...
			<DIV id="whats_hot">
				...
				<TR>
					...
					<?PHP
						...
						{
							echo '<DIV class="old_price">'.$thisItem["price"].get_setting($mysqli, "es_currency", "value").'</DIV>
							<DIV class="price">'.((int)$thisItem["price_percent"]*(int)$thisItem["price"]/100).get_setting($mysqli, "es_currency", "value").'</DIV>';
						}
						else
						{
							echo '<DIV class="price">'.((int)$thisItem["price_percent"]*(int)$thisItem["price"]/100).get_setting($mysqli, "es_currency", "value").'</DIV>';
						}
					?>
						<DIV class='actions'>
							Sold by:
							...
							?>
							<BR /><?PHP echo"<A HREF='item.php?item=".$_GET['item']."&add=".$thisItem['id']."' id='addtocart'>"; ?> Add to Cart </A>
						</DIV>
					</TD>
			</TR>
			<TR>
